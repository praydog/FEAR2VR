    ...
    for n in range(cap):
        ...
        if n > 0:
            ...
            if outside:
                ...
                sig = "TABLE_START" if stores else "REFERENCED"
                src = stores or outside
                candidates.append((n, sig, "from " + ", ".join(
                    "0x%08X (%s)" % (a, idc.GetDisasm(a).split(";")[0].strip()[:44]) for a in src[:2])))
