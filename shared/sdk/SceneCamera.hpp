...
struct SceneCameraSnapshot {
    ...
    std::array<float, 12> view{};             // 3 rows of 4
    ...
    // [3][3] == 1. THIS IS THE PRECONDITION A CONSUMER NEEDS before treating half_view_plane as
    // tan(fov/2): that reading is meaningless for the screen pass, and this is how to tell the
    // passes apart from the data rather than by trusting `mode`.
